...
```
#on master-node
### Solution: Disable swap on the master node:
sudo swapoff -a
sudo sed -i '/swap/d' /etc/fstab
sudo systemctl restart kubelet

### Ensure all necessary ports (e.g., 6443) are open:
sudo ufw allow 6443
sudo ufw reload

sudo systemctl restart kubelet

#Copy the admin.conf File from Master Node to Worker Node:
##on master-node
master@master-node:~$ sudo scp /etc/kubernetes/admin.conf worker02@worker02:/home/worker02/admin.conf
##on worker-nodes
worker01@worker01:~$ mkdir -p $HOME/.kube
worker01@worker01:~$ cp /home/worker01/admin.conf $HOME/.kube/config
worker01@worker01:~$ sudo chown worker01:worker01 /home/worker01/admin.conf
worker01@worker01:~$ export KUBECONFIG=/home/worker01/admin.conf
worker01@worker01:~$ echo 'export KUBECONFIG=/home/worker01/admin.conf' >> ~/.bashrc
worker01@worker01:~$ source ~/.bashrc



sudo scp vagrant@masternode:/home/vagrant/admin.conf /etc/kubernetes/admin.conf
